...
create table if not exists public.profiles (
  ...
  booking_url   text,                          -- doctor's external appointment link (optional)
  created_at    timestamptz not null default now()
);

-- physiotherapist opens free slots; a patient books one (internal appointment system)
...
-- web push subscriptions (one per browser/device); server sends reminders via VAPID
...
create table if not exists public.exercises (
  ...
  freq int default 1,          -- sessions required per day (prove once per session)
  note        text,
...
-- appointment_slots: doctor manages own; patient sees their doctor's slots + books an open one
...
-- a patient may book an open slot offered by their own physiotherapist (claim only)
...
-- (instant doctor→patient program/note/appointment sync). Safe: Realtime honors RLS.
do $$ begin
